...
> = {
  ELIGIBLE: {
    title: "Ön deðerlendirme olumlu görünüyor",
    tone: "border-emerald-200 bg-emerald-50 text-emerald-950",
    description:
      "Backend motoru mevcut bilgilerle uygunluk yönünde bir ön karar üretti. Bu sonuç resmi hak sahipliði kararý deðildir.",
    nextStepTitle: "Sonraki adým",
    nextStepBody:
      "Baþvuru öncesinde gelir, hane ve bakým koþullarýna iliþkin belgelerinizi düzenli biçimde hazýrlamanýz faydalý olur.",
  },
  NOT_ELIGIBLE: {
    title: "Ön deðerlendirme olumsuz görünüyor",
    tone: "border-rose-200 bg-rose-50 text-rose-950",
    description:
      "Backend motoru girilen bilgilerle uygunluk yönünde sonuç üretmedi. Resmi deðerlendirme için kurum incelemesi esastýr.",
    nextStepTitle: "Sonraki adým",
    nextStepBody:
      "Girilen bilgileri tekrar kontrol edin. Özellikle gelir, hane kiþi sayýsý ve diðer temel alanlarýn doðru olduðundan emin olun.",
  },
  NEEDS_INFO: {
    title: "Daha fazla bilgi gerekli",
    tone: "border-amber-200 bg-amber-50 text-amber-950",
    description:
      "Mevcut bilgiler karar vermek için yeterli deðil. Eksik alanlarý tamamladýktan sonra yeniden deneyebilirsiniz.",
    nextStepTitle: "Sonraki adým",
    nextStepBody:
      "Eksik görünen bilgileri tamamlayýn ve ardýndan yeniden ön deðerlendirme alýn. Sonuç ekranýndaki eksik bilgi listesi bunun için rehberdir.",
  },
};
...
function resultPrimaryAction(status: EligibilityStatus) {
  ...
  if (status === "ELIGIBLE") {
    return {
      label: "Rehberde baþvuru hazýrlýðýný gözden geçir",
      href: "/evde-bakim-maasi",
    };
  }

  return {
    label: "Þartlarý rehber sayfasýnda tekrar incele",
    href: "/evde-bakim-maasi",
  };
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err instanceof ApiClientError) {
        ...
        setError("Beklenmeyen bir hata oluþtu. Lütfen daha sonra tekrar deneyin.");
      }
    } finally {
    ...
    <main className="min-h-screen px-6 py-12 lg:px-10 lg:py-16">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="card-panel">
          <p className="eyebrow">Hesaplama Aracý</p>
          <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
            Evde bakým maaþý için backend destekli ön deðerlendirme
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-8 text-slate-700">
            Bu araç frontend tarafýnda uygunluk hesabý yapmaz. SocialRightOS backend karar
            motoruna canonical endpoint üzerinden istek gönderir ve sonucu açýklayýcý biçimde sunar.
          </p>

          <div className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-700">
            Formda yalnýzca gerekli temel bilgiler istenir. Kimlik numarasý, açýk adres veya belge
            yükleme bu aþamada istenmez.
          </div>

          <div id="form-start" className="mt-8 grid gap-5 md:grid-cols-2">
            <label className="form-field">
              <span>Engellilik oraný</span>
              <input
                type="number"
                min="0"
                max="100"
            ...
            <label className="form-field">
              <span>Toplam hane geliri (aylýk)</span>
              <input
                type="number"
                min="0"
            ...
            <label className="form-field">
              <span>Hanedeki kiþi sayýsý</span>
              <input
                type="number"
                min="1"
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-medium text-slate-900">Temel doðrulamalar</p>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            >
              {isSubmitting ? "Deðerlendiriliyor..." : "Ön deðerlendirmeyi çalýþtýr"}
            </button>
            <button
              type="button"
              onClick={() => {
          ...
          {error ? (
            <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900">
              <p className="font-semibold">Ýstek tamamlanamadý</p>
              ...
              {hasConfigError ? (
                <p className="mt-3 leading-7">
                  Frontend deploy ortamýnda backend base URL tanýmlanmadan bu araç canlýya
                  alýnmamalý.
                </p>
              ) : null}
            </div>
          ...
          {result && statusPanel ? (
            <section className={`mt-6 rounded-3xl border p-6 ${statusPanel.tone}`}>
              ...
              {ruleResults.length > 0 ? (
                <div className="mt-5 rounded-2xl bg-white/70 p-4">
                  <h3 className="font-semibold">Kural sonuçlarý</h3>
                  <ul className="mt-3 space-y-3 text-sm leading-7">
              ...
              <div className="mt-5 rounded-2xl bg-white/70 p-4">
                <h3 className="font-semibold">Deðerlendirme metadata</h3>
        ...
        <aside className="space-y-6">
          <div className="card-panel">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700">
              Bu araç resmi kurum kararý yerine geçmez. Sonuçlar yalnýzca baþvuru öncesi ön
              deðerlendirme ve bilgi amaçlýdýr.
            </p>
          </div>

          <div className="card-panel">
            <h2 className="text-lg font-semibold text-slate-950">Veri yaklaþýmý</h2>
            <p className="mt-3 text-sm leading-7 text-slate-700">
              Kimlik numarasý, açýk adres veya belge yükleme istenmez. MVP yalnýzca gerekli
              temel deðerlendirme alanlarýný kullanýr.
            </p>
          </div>

          <div className="card-panel">
            <h2 className="text-lg font-semibold text-slate-950">Hýzlý hazýrlýk listesi</h2>
            <ul className="mt-3 space-y-2 text-sm leading-7 text-slate-700">
              <li>Gelir bilgisini netleþtirin.</li>
              <li>Hanedeki kiþi sayýsýný doðru girin.</li>
              <li>Eksik sonuç aldýysanýz gerekli alanlarý tamamlayýn.</li>
            </ul>
          </div>
        </aside>
      </div>
